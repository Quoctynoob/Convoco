...
export default function DebateLobbyPage() {
    ...
    const unsubscribe = onSnapshot(
      ...
      async (docSnapshot) => {
        if (docSnapshot.exists()) {
          const debateData = { id: docSnapshot.id, ...docSnapshot.data() } as Debate;
          ...
          if (debateData.creatorReady && debateData.opponentReady && countdown === null) {
            setCountdown(3);
          }
          ...
          try {
            ...
            if (!opponent && debateData.opponentId) {
              const opponentProfile = await getUserProfile(debateData.opponentId);
        ...
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-600 mb-6">
              Unable to load the debate lobby. It may have been deleted or you might not have permission to view it.
            </p>
            <Link href="/debates">
  ...
  const readyCount = (debate.creatorReady ? 1 : 0) + (debate.opponentReady ? 1 : 0);
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-6">
          <div className="text-center mb-8">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Debate Lobby</h2>
            <p className="text-gray-600">
              Both participants must click "Ready" to begin the debate.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {/* Creator card */}
            <div className={`p-4 rounded-lg border-2 ${debate.creatorReady ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
              <div className="flex items-center gap-3 mb-2">
                {creator?.photoURL ? (
                  <img src={creator.photoURL} alt={creator.username} className="h-10 w-10 rounded-full" />
                ) : (
                ...
                <div>
                  <p className="font-semibold">{creator?.username || 'Creator'}</p>
                  <p className="text-sm text-gray-500">
                    {debate.creatorSide === 'affirmative' ? 'Affirmative Side' : 'Negative Side'}
                  </p>
                </div>
              </div>
            ...
            {/* Opponent card */}
            <div className={`p-4 rounded-lg border-2 ${debate.opponentReady ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
              <div className="flex items-center gap-3 mb-2">
                {opponent?.photoURL ? (
                  <img src={opponent.photoURL} alt={opponent.username} className="h-10 w-10 rounded-full" />
                ) : (
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {debate.creatorSide === 'affirmative' ? 'Negative Side' : 'Affirmative Side'}
                  </p>
                </div>
              </div>
              <div className="mt-3 text-center">
          ...
          <div className="text-center">
            {countdown !== null ? (
              <div className="bg-indigo-100 rounded-lg p-8 flex flex-col items-center justify-center">
                <h3 className="text-xl font-bold text-indigo-800 mb-2">Debate Starting</h3>
                <div className="text-6xl font-bold text-indigo-700 mb-2">{countdown}</div>
                <p className="text-indigo-600">Get ready to debate!</p>
              </div>
            ) : (
              <>
                <div className="mb-6">
                  <div className="inline-block bg-gray-100 rounded-full px-4 py-2 mb-4">
                    <span className="text-gray-700 font-medium">{readyCount}/2 participants ready</span>
                  </div>
                  {readyCount === 2 && (
                ...
                <p className="mt-2 text-sm text-gray-500">
                  {isReady
                    ...
                    : "Click when you're ready to start the debate"}
                </p>
              </>
            )}
          </div>
